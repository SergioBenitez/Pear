...
impl<'a> Authority<'a> {
    ...
    fn new(user_info: Option<&'a str>, host: &'a str, port: Option<u16>) -> Authority<'a> {
        Authority {
            source: None,
    ...
    match input.len() {
        ...
        1 => switch! {
            ...
            _ => unsafe {
                ...
                let host = take_n_while(1, is_reg_name_char)?;
...
fn port<'a>(input: &mut RawInput<'a>) -> Result<'a, u16> {
    ...
    for (b, i) in port_str.to_source(&Some(input.cow_source())).iter().rev().zip(&[1, 10, 100, 1000, 10000]) {
...
pub fn main() {
    let uri = parse_bytes(&b"ftp:a/b?c=d"[..]);
    match uri {
...
mod test {
    ...
    fn authority() {
        assert_parse_eq!(
            "sergio:benitez@spark" => Authority::new(Some("sergio:benitez"), "spark", None),
            "a:b:c@1.2.3:12121" => Authority::new(Some("a:b:c"), "1.2.3", Some(12121)),
            "sergio@spark" => Authority::new(Some("sergio"), "spark", None),
            "sergio@spark:230" => Authority::new(Some("sergio"), "spark", Some(230)),
            "sergio@[1::]:230" => Authority::new(Some("sergio"), "1::", Some(230)),
        );
    }
    ...
    fn absolute() {
        assert_parse_eq!(
            "http://foo.com:8000" => Absolute::new(
                "http",
                Some(Authority::new(None, "foo.com", Some(8000))),
                None
            ),
            "http://foo:8000" => Absolute::new(
                "http",
                Some(Authority::new(None, "foo", Some(8000))),
                None,
            ),
            ...
            "http://sergio:pass@foo.com:8000" => Absolute::new(
                "http",
                Some(Authority::new(Some("sergio:pass"), "foo.com", Some(8000))),
                None,
            ),
            ...
            "http://foo.com?test" => Absolute::new(
                "http",
                Some(Authority::new(None, "foo.com", None,)),
